                ...
                {selected.messages?.map((msg: any) => (
                  <div
                    ...
                    className={`rounded-xl px-4 py-3 text-sm ${
                      msg.is_staff
                        ? "bg-[#FFE100]/10 border border-[#FFE100]/20 ml-4"
                        : "bg-[#1C1B1B] mr-4"
                    }`}
                  >
              ...
              {selected.status !== "closed" && (
                <div className="p-4 border-t border-white/5 flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-[#1C1B1B] border border-white/5 text-white text-sm px-4 py-2.5 rounded-xl focus:outline-none focus:border-[#FFE100]/40"
                    onKeyDown={(e) => e.key === "Enter" && handleReply()}
                  />
                  <button
                    onClick={handleReply}
                    className="px-4 py-2.5 bg-[#FFE100] text-[#211C00] font-bold text-xs rounded-xl uppercase"
                  >
                    Ответить
                  </button>
                </div>
