...
function Countdown({ endsAt }: { endsAt: string }) {
  ...
  useEffect(() => {
    const calc = () => {
      ...
      setDisplay(d > 0 ? `${d}д ${h}ч` : h > 0 ? `${h}ч ${m}м` : `${m}м`);
    };
    calc();
  ...
  return (
    <div className="text-right">
      ...
      <div className="text-[#FFE100] font-black text-sm">{display}</div>
    </div>
  );
}
...
export default function DashboardPage() {
  ...
  const loadData = useCallback(() => {
    Promise.all([
      ...
      .then(([g, s, p]) => {
        const giveawayData: Giveaway[] = g.data;
        setGiveaways(giveawayData);
        setStats(s.data);
        setLastPrize(p.data?.[0] || null);
        // Загружаем участия из бэкенда
        const participatingIds = new Set<string>(
          giveawayData
            .filter((gw: any) => gw.is_participant)
            .map((gw: any) => gw.id)
        );
        setJoined(participatingIds);
      })
      .catch(() => {})
  ...
  const handleClipSubmit = async () => {
    ...
    try {
      ...
      setTimeout(() => { setClipModal(false); setClipSuccess(false); }, 2000);
    } catch (err: any) {
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: "Активных",  value: stats?.active_giveaways ?? "—",      color: "text-white" },
          {
            ...
            color: (stats?.violations ?? 0) > 0 ? "text-red-400" : "text-green-400",
          },
        ].map((item) => (
          <div key={item.label}
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 space-y-4">
          ...
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[1, 2].map((i) => (
                <div key={i} className="bg-[#111] border border-white/5 rounded-2xl overflow-hidden animate-pulse">
                  <div className="h-48 bg-[#1C1B1B]" />
                  <div className="p-5 space-y-3">
                    <div className="h-4 bg-[#1C1B1B] rounded w-3/4" />
            ...
            <div className="bg-[#111] border border-white/5 rounded-2xl p-8 text-center">
              <span className="material-symbols-outlined text-white/10 text-5xl block mb-3">redeem</span>
              <p className="text-white/40 text-sm">Сейчас нет активных розыгрышей</p>
              <p className="text-white/20 text-xs mt-1">Следи за анонсами в Telegram</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {giveaways.map((g) => {
                ...
                return (
                  <div key={g.id}
                    className="group bg-[#1C1B1B] hover:bg-[#222] rounded-2xl overflow-hidden transition-all border border-white/5 relative">
                    ...
                    {/* Фото */}
                    <div className="h-48 bg-[#0E0E0E] overflow-hidden">
                      {g.skin_image_url ? (
                        <img src={g.skin_image_url} alt={g.title}
                          className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center">
                          <span className="material-symbols-outlined text-white/5" style={{ fontSize: "64px" }}>redeem</span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="p-5">
                      <div className="flex justify-between items-start mb-3">
                        <div className="flex-1 min-w-0 pr-3">
                          <h3 className="font-bold text-white leading-tight truncate">{g.title}</h3>
                          {g.skin_name && (
                            <p className="text-xs text-white/40 mt-0.5 truncate">{g.skin_name}</p>
                          )}
                        </div>
                        {g.ends_at && <Countdown endsAt={g.ends_at} />}
                      </div>
                        ...
                        <button
                          ...
                          className={`px-3 py-1.5 text-xs font-bold rounded-lg uppercase transition-colors disabled:opacity-50 ${
                            isJoined
                              ...
                              : "bg-[#FFE100] text-[#211C00] hover:bg-[#FFE330]"
                          }`}
                        >
        ...
        <div className="w-full lg:w-72 flex flex-col gap-6">
              ...
              <div className="bg-[#111] border border-white/5 rounded-2xl p-5 border-l-4 border-l-[#FFE100]">
                <div className="relative h-40 bg-[#0E0E0E] rounded-xl mb-4 overflow-hidden">
                  {lastPrize.skin_image_url ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      <span className="material-symbols-outlined text-white/10" style={{ fontSize: "48px" }}>inventory</span>
                    </div>
                  )}
                  <div className="absolute bottom-2 right-2">
                    <span className={`text-[9px] font-black px-2 py-0.5 rounded uppercase ${
                      PRIZE_STATUS[lastPrize.status]?.color || "text-white/40"
                    } bg-black/60`}>
                      {PRIZE_STATUS[lastPrize.status]?.label || lastPrize.status}
                    </span>
                  </div>
                </div>
                <h4 className="font-bold text-sm text-white mb-0.5">{lastPrize.name}</h4>
                <p className="text-[10px] text-white/40 uppercase tracking-widest mb-3">
                  {lastPrize.created_at?.slice(0, 10)}
                </p>
                <button onClick={() => navigate("/dashboard/prizes")}
                  className="w-full py-2 bg-[#1C1B1B] hover:bg-[#2A2A2A] text-xs font-bold uppercase tracking-widest transition-colors rounded-xl">
                  Все призы
                </button>
              </div>
            ) : (
              <div className="bg-[#111] border border-white/5 rounded-2xl p-5 text-center">
                <span className="material-symbols-outlined text-white/10 text-4xl block mb-2">inventory</span>
                <p className="text-white/30 text-sm">Призов пока нет</p>
                <p className="text-white/20 text-xs mt-1">Участвуй в розыгрышах!</p>
              </div>
            )}
          </div>
          ...
          {/* Безопасность */}
          <div>
            <h2 className="text-xl font-black uppercase tracking-tight mb-4">Безопасность</h2>
            {(stats?.violations ?? 0) > 0 ? (
              <div className="bg-red-500/10 border-l-4 border-red-500 p-5 rounded-r-2xl">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-red-400 font-black text-lg uppercase">{stats.violations} нарушения</span>
                </div>
                <p className="text-xs text-red-400/70 uppercase font-medium leading-relaxed">Проверь раздел нарушений</p>
                <button onClick={() => navigate("/dashboard/violations")}
              ...
              <div className="bg-green-500/10 border-l-4 border-green-500 p-5 rounded-r-2xl">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-green-400 font-black text-lg uppercase">Нарушений нет</span>
                </div>
          ...
          <button onClick={() => setClipModal(true)}
            ...
            <span className="material-symbols-outlined text-2xl text-[#FFE100]">video_library</span>
            <span className="text-sm font-bold text-[#FFE100] uppercase tracking-widest">Предложить клип</span>
          </button>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
          <div className="bg-[#111] border border-white/10 rounded-2xl p-8 w-full max-w-md space-y-5">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-black uppercase tracking-tight">Предложить клип</h2>
              <button onClick={() => { setClipModal(false); setClipError(""); setClipSuccess(false); }}
            ...
            {clipSuccess ? (
              <div className="bg-green-500/10 border border-green-500/20 text-green-400 px-4 py-6 rounded-xl text-center">
                ...
                <p className="text-sm mt-1 text-green-400/60">Мы рассмотрим его в ближайшее время</p>
              </div>
            ) : (
              <>
                {clipError && (
                ...
                <div className="space-y-4">
                  {[
                    { field: "title", label: "Название *",        placeholder: "Эпичный клатч 1 vs 5" },
                    { field: "url",   label: "Ссылка на клип *",  placeholder: "https://clips.twitch.tv/..." },
                    { field: "game",  label: "Игра",              placeholder: "CS2, Valorant..." },
                  ].map((item) => (
                    <div key={item.field}>
